pipeline {
    ...
    stages {
        stage('PROD') {
            ...
            steps {
              sh '''#!/bin/bash
                    git credentialsId: 'd017afeb-dcf8-4f4a-b73c-07d793e4628a', url: 'https://github.com/KuzRomanDocker/Task2.git'
                    git clone https://github.com/KuzRomanDocker/Task3.git
                    MSG=$(git log -1 --format=%B)
                    mkdir PROD
                    rsync -avr --exclude='.git' --exclude='.github' --exclude='Jenkinsfile' --exclude='README.md' --delete Task3/. PROD
                    cd PROD
